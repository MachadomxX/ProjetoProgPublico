...
def atualizar(classe:str):
    try:
        ...
        if 'id' not in dados:
            ...
            elif classe == "Fucionario":id = dados['id'];funcionario = Funcionario.query.get(id);resposta = jsonify({"resultado": "erro", "detalhes": "Objeto não encontrado"}) if funcionario is None else jsonify({"resultado": "ok", "detalhes": "ok"});funcionario.salario = dados['salario'];funcionario.cargaH = dados['cargaH'];escola.cargos = dados['cargos'];db.session.commit();return resposta
            elif classe == "Estudante":id = dados['id'];estudante = Estudante.query.get(id);resposta = jsonify({"resultado": "erro", "detalhes": "Objeto não encontrado"}) if estudante is None else jsonify({"resultado": "ok", "detalhes": "ok"});estudante.matricula = dados['matricula'];estudante.senha = cifrar(dados['senha']);db.session.commit();return resposta
